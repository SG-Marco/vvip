...
import numpy as np
from coordinator import PadPrompter, PROGRAM, Coordinator  # coordinator.py에서 클래스 임포트
...
# DeltaLearner 초기화
delta_learner = PROGRAM(args=None).to(device)  # PROGRAM 사용
...
for step, batch in enumerate(processed_dataset):
    # 데이터 준비
    mel = batch["mel"].to(device)  # 1채널 유지
